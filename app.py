...
.pill strong { color: #e2e8f0; font-weight: 600; }
...
# ---------------------------------------------------------------------------
# Tab callback and top navigation
# ---------------------------------------------------------------------------
...
def _pill(label: str, value: str) -> str:
    return f'<div class="pill">{label} <strong>{value}</strong></div>'
...
def render_today():
    ...
    st.markdown(
        f'<h2 style="font-size:clamp(1.5rem,4vw,2rem);font-weight:600;'
        f'letter-spacing:-0.3px;margin:0 0 0.15rem 0;line-height:1.2;">'
        f'Good morning, {first_name}</h2>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<p class="r-caption">{date.today().strftime("%A, %B %-d, %Y")}</p>',
        unsafe_allow_html=True,
    )

    # Metric pills (no emojis — clean text labels)
    def fv(key): return today_row.get(key)
    pills = []
    if fv("resting_hr"):       pills.append(_pill("HR",      f"{int(fv('resting_hr'))} bpm"))
    if fv("sleep_score"):      pills.append(_pill("Sleep",   str(int(fv("sleep_score")))))
    if fv("hrv_last_night"):   pills.append(_pill("HRV",     f"{int(fv('hrv_last_night'))}ms"))
    if fv("body_battery_high"): pills.append(_pill("Battery", str(int(fv("body_battery_high")))))
    if fv("steps"):            pills.append(_pill("Steps",   f"{int(fv('steps')):,}"))
    if pills:
    ...
    with col_btn:
        refresh_clicked = st.button("Refresh", key="refresh_today")
    ...
    # Insight cards — each as an expandable card
    ...
    if insight_list:
        for card in insight_list:
            ...
            with st.expander(label, expanded=False):
                st.markdown(
                    f'<p style="color:#e2e8f0;line-height:1.75;'
                    f'font-size:0.9rem;margin:0;">{detail}</p>',
                    unsafe_allow_html=True,
                )
    ...
    if questions:
        st.markdown('<p class="r-caption" style="margin-top:0.75rem;">Follow-up questions</p>',
                    unsafe_allow_html=True)
...
def render_patterns():
    ...
    with col_btn:
        refresh_clicked = st.button("Refresh", key="refresh_patterns")
    ...
    if analysis_text:
        st.markdown(
            f'<div class="r-card">{analysis_text}</div>',
            unsafe_allow_html=True,
        )
...
def render_history():
    ...
    with info_col:
        st.markdown(
            f'<p class="r-caption" style="padding-top:0.5rem;">'
            f'Last loaded {datetime.now().strftime("%-I:%M %p")}</p>',
            unsafe_allow_html=True,
        )
    ...
    st.divider()
    st.markdown(
        f'<p class="r-caption">Signed in as {_user.get("email", "")}</p>',
        unsafe_allow_html=True,
    )
